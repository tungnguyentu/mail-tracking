...
/** Quiet-desk tokens shared with web brand. */
...
function ensureToggle(composeRoot: HTMLElement) {
 ...
 label.title = "Track engagement (sticky preference)";
...
async function onSendClick(ev: Event) {
 ...
 if (!prefs?.apiToken) {
 console.info("[MailTrack] Sign in / set API token to track sends.");
 return;
 }
 ...
 console.warn("[MailTrack] tracking failed; sending untracked", e);
 }
}
